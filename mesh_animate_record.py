...
class mesh_animate(object):
    def __init__(self,lonlatbox=(-76.6,36.4,-75.5,40.0)):
...
#         def get_particles_pp(self,pp,pcolors):
    ...
    def run_animation(self,xmesh=False,setinterval=10):
        ...
        # as the rainparticles develop.
        self.scat = self.ax.scatter(self.particles[1][:, 0], self.particles[1][:, 1],
                  s=10, lw=0.5, facecolors=self.colorspp)
        #self.scat.set_edgecolors(self.colorspp)
        
        # Construct the animation, using the update function as the animation director.
        #anim_part = FuncAnimation(self.fig, self.update, interval=setinterval)
        anim_part = FuncAnimation(self.fig, self.update, frames=self.n_time,interval=setinterval)
        anim_part.save('ches.mp4',writer=writer)
        plt.show()
